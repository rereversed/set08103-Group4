...
public class World {
    ...
    public void getCountriesByPopulation(Connection con) { //1
        if (con == null) {
    ...
    public void getTopNCountriesByPopulation(Connection con, int n) { //4
        if (n < 1) {
    ...
    public void getCitiesByPopulation(Connection con) { //7
        if (con == null) {
            System.out.println("Connection is null.");
            return;  // Exit the method if there is no connection
        }
        ...
        try (Statement stmt = con.createStatement();
             ResultSet rs = stmt.executeQuery(query)) {
            System.out.println("Cities by Descending Population:");
            while (rs.next()) {
                String name = rs.getString("name");
    ...
    public void getTopNCitiesByPopulation(Connection con, int n) { //12
        if (n < 1) {
            System.out.println("N must be at least 1.");  // Ensuring N is positive
            n = 1;
        }
    ...
    public void getCapitalCitiesByPopulation(Connection con) { //17
        if (con == null) {
            System.out.println("Connection is null.");
            return;  // Exit the method if there is no connection
        }
